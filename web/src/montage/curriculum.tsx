...
export default function Curriculum(){
 ...
 useEffect(()=>{const abort=new AbortController();fetch(`${basePath}/curriculum-montage/index.json`,{signal:abort.signal,cache:"no-store"}).then(r=>{if(!r.ok)throw new Error("Recordings unavailable");return r.json();}).then(setIndex).catch(e=>{if(!abort.signal.aborted)setError(String(e));});return()=>abort.abort();},[]);
 ...
 useEffect(()=>{const abort=new AbortController();const urls=JSON.parse(key) as string[];Promise.all(urls.map(url=>fetch(`${basePath}${url}`,{signal:abort.signal,cache:"no-store"}).then(r=>{if(!r.ok)throw new Error("Could not load scenario recordings");return r.json() as Promise<Trace[]>;}))).then(groups=>{setLoaded({key,traces:groups.flat()});setError("");}).catch(e=>{if(!abort.signal.aborted)setError(String(e));});return()=>abort.abort();},[key]);
