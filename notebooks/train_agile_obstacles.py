...
from notebooks.agile_obstacles import AgileFeatures, PROFILES, expert, reset_course, DroneEnv
...
def run(output, episodes=12, epochs=100, rounds=3, ppo_steps=8192, max_level=7, final_cases=16):
    ...
    for name in ("train_agile_obstacles.py", "agile_obstacles.py", "agile_v4.py", "agile_v2.py", "agile_curriculum.py", "train_imitation.py"):
    ...
    dump(output/"configuration.json", dict(profiles=PROFILES, episodes=episodes, epochs=epochs,
         ...
         original_fingerprint=fingerprint(), promotion="8/8 selection cases in hover and every level through the current level",
         observation="109 float32: previous-gate 46 plus 9 nearest-first other-gate slots; each displacement/200 body (3), body normal (3), presence (1)", inference="neural actor only; no expert/planner calls",
         timeout_s={0:5,1:60,3:180,10:600}, seed=2031,
         extracted_features=138, inherited_level=4, first_training_level=5,
         transfer_checkpoint="runs/agile-curriculum-004/accepted-model.zip",
         transfer_sha256=hashlib.sha256(Path("runs/agile-curriculum-004/accepted-model.zip").read_bytes()).hexdigest(),
         ...
         retained_expert_levels=[1,2,3,4], final_seed_base=240000000,
         selection_cases=8, mixed_straight_leg_obstacle_filter=False,
         planner_frame_inflation_m=.8, obstacle_dimensions="fixed standard gate dimensions; yaw only"))
    ...
    with np.load("runs/imitation-repair/demonstrations.npz") as data:
        ...
        observations = list(np.pad(data["observations"][keep], ((0,0),(0,69))))
        labels = list(data["actions"][keep])
    history = []; mastered = 4; total_ppo = 0
    transfer = PPO.load("runs/agile-curriculum-004/accepted-model.zip",device="cpu")
    state = model.policy.state_dict()
    for key, value in transfer.policy.state_dict().items():
        if state[key].shape == value.shape: state[key] = value.clone()
        elif key.endswith(".0.weight") and state[key].shape[1] == 138:
            state[key].zero_(); state[key][:,:39] = value
    model.policy.load_state_dict(state)
    model.save(output/"accepted-model.zip")
    try:
        for retained_level in (1,2,3,4):
            collect(model,retained_level,0,8,output,observations,labels)
        for level in range(5,max_level+1):
            retained = [-1]+list(range(level+1))
            accepted = False
            for round_index in range(rounds):
                ...
                scores = assess(model, retained, 8, 230000000, output, f"selection-{level}-{round_index}")
            ...
            ppo_scores = assess(model, retained, 8, 230000000, output, f"ppo-selection-{level}")
        ...
        validation = assess(model, [-1]+list(range(max_level+1)), final_cases, 240000000, output, "final-validation")
        original = PPO.load("runs/imitation-repair/iteration-0/model.zip", device="cpu")
        baseline = assess(original, [-1]+list(range(max_level+1)), final_cases, 240000000, output, "original-baseline")
