...
from notebooks.agile_v3 import AgileFeatures, PROFILES, expert, reset_course, DroneEnv
...
def run(output, episodes=12, epochs=100, rounds=3, ppo_steps=8192, max_level=7, final_cases=16):
    ...
    for name in ("train_agile_v3.py", "agile_v3.py", "agile_v2.py", "agile_curriculum.py", "train_imitation.py"):
    ...
    transfer = PPO.load("runs/agile-curriculum-001/level-3-round-0/model.zip",device="cpu")
    state = model.policy.state_dict()
    for key, value in transfer.policy.state_dict().items():
        if state[key].shape == value.shape: state[key] = value.clone()
        elif key.endswith(".0.weight") and state[key].shape[1] == 31:
            state[key].zero_(); state[key][:,:25] = value
    ...
    try:
        ...
        validation = assess(model, [-1]+list(range(max_level+1)), final_cases, 180000000, output, "final-validation")
        original = PPO.load("runs/imitation-repair/iteration-0/model.zip", device="cpu")
        baseline = assess(original, [-1]+list(range(max_level+1)), final_cases, 180000000, output, "original-baseline")
